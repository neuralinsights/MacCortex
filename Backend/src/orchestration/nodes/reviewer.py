...
"""
    ...
    async def review(self, state: SwarmState) -> SwarmState:
        ...
        review_result = await self._review_with_llm(
            ...
            subtask_description=subtask["description"],
            state=state,  # Phase 5: 传递 state 用于 Token 追踪
        )
...
"""
        ...
        if self.router is not None:
            ...
            # 追踪 Token 使用量（如果提供了 state）
            if state is not None:
                updated_state = update_token_usage(
                    state=state,
                    agent_name="reviewer",
            ...
            print(f"[Reviewer] Token 使用: input={llm_response.usage.input_tokens}, "
