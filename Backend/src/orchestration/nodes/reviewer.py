...
输出格式：
- 必须输出 JSON 格式
- 字段：{"passed": true/false, "feedback": "反馈内容"}
- passed 为 true 时，feedback 可以为空或简短确认
- passed 为 false 时，feedback 必须详细且可操作
...
{
  "passed": false,
  "feedback": "代码在第 5 行抛出 ZeroDivisionError。问题：除法函数缺少除零检查。修复方案：在 divide 函数开头添加：if b == 0: raise ValueError('除数不能为零')"
}
```
"""
